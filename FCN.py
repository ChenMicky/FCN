...
def vgg_net(weights, image,FLAGS):
    ...
    )  ## Note tuple can't be modfied ,adversely , list can  do it.
...
def inference(image, keep_prob,FLAGS):
    """
...
def train(loss,global_step,FLAGS):
    ...
    gardinets =optimizer.compute_gradients(loss)
    apply_gradient_op = optimizer.apply_gradients(gardinets,global_step=global_step)    

    for variables in tf.trainable_variables():
        tf.summary.histogram(variables.op.name,variables)

    for grad,var in gardinets:
        if grad is not None:
...
def _add_loss_summeries(total_loss):## it be used with weight decay
    loss_averages = tf.train.ExponentialMovingAverage(0.999)
